...
class GazeboEnvSingleAgent(Env):

    def __init__(self, odom_subscribers: List[OdomSubscriber], scan_subscribers: List[ScanSubscriber], goal_position=(0., 0.), test=False,temp_agent_index = 0, agent_count = 1 ,nodename = 'GazeboEnv2',alloted_bot = 1):
        super(GazeboEnvSingleAgent, self).__init__()

        self.odom_subscribers = odom_subscribers
        self.scan_subscribers = scan_subscribers
        self.test = test
        self.agent_index = temp_agent_index
        self.alloted_bot = alloted_bot
        ...
        topic_name = f"/robot_{self.agent_index+1}/cmd_vel"
        self.vel_pubs[self.agent_index] = self.node.create_publisher(Twist, topic_name, 10)
        ...
        self.last_actions[self.agent_index] = (0.0, 0.0)
        distance = Utils.get_distance_to_goal(self.get_robot_position(self.agent_index), self.goal_position)
        self.prev_distances_to_goal.append(distance)
    ...
    def change_goal_position(self):
        ...
        def step(self, action_n: List):
            ...
            for i in range(self.agent_count):
                linear_x = float(action_n[self.a][0])
    ...
    def check_done(self):
        ...
        done_tuple = self.check_done_id(self.agent_index)
    ...
    def get_reward(self, done_n, collision_n, target_n, action_n, min_laser_reading_n):
        ...
        # total_approach_reward = self.calculate_approach_reward()
            reward_i = self.get_reward_id(
                done_n[self.agent_index], collision_n[i], target_n[i], action_n[i], min_laser_reading_n[i], self.agent_index)
    ...
    def calculate_approach_reward(self):
        ...
        current_distance_to_goal = Utils.get_distance_to_goal(
            self.get_robot_position(self.agent_index), self.goal_position)

        approach_dist = self.prev_distances_to_goal[self.agent_index] - \
