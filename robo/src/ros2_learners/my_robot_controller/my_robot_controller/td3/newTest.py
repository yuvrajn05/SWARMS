...
from GazeboEnv1 import GazeboEnvSingleAgent
...
def main():
    ...
    goal_position = (x_rounded, y_rounded)

    real_goal_posiion = (x_rounded, y_rounded)


    print(goal_position)

    namespaces = [f"robot_{i+1}" for i in range(AGENT_COUNT) if i != 4]  # Skip robot_5, which is at index 4
    ...
    for i, namespace in enumerate(namespaces):
        ...
        # Add nodes only if the robot is not 'robot_5'
        if namespace != "robot_5":
    ...
    env = GazeboEnvMultiAgent(odom_subscribers=odom_subscribers, scan_subscribers=scan_subscribers,
                              goal_position=goal_position, test=True, agent_count = AGENT_COUNT ,nodename = 'GazeboEnv')
    ...
    scan_subscribers2 = []
    robot_index = 4
    odom_subscriber2 = OdomSubscriber("robot_5", 4)
    scan_subscriber2 = ScanSubscriber("robot_5", 4)
    ...
    env2 = GazeboEnvSingleAgent(odom_subscribers=odom_subscribers2, scan_subscribers=scan_subscribers2,
                              goal_position=goal_position, test=True, agent_count = 1 ,nodename = 'GazeboEnv2')
